...
BIN="$REPO_ROOT/bin/kanon-client"

SERVER="${SERVER:-http://127.0.0.1}"
LOCAL_ROOT="${LOCAL_ROOT:-$HOME/Documents/kanon}"
STATE_DIR="${STATE_DIR:-$HOME/Library/Application Support/kanon}"
...
PRINT_PATH_LIMIT="${PRINT_PATH_LIMIT:-50}"
SYNC_MODE="${SYNC_MODE:-auto}"
RSYNC_SOURCE="${RSYNC_SOURCE:-server-host:/root/docs/}"
...
if [[ "$BUILD_ON_RUN" == "1" || ! -x "$BIN" ]]; then
  (cd "$REPO_ROOT" && go build -o "$BIN" ./cmd/kanon-client)
fi
...
ARGS=(
  ...
  -print-path-limit "$PRINT_PATH_LIMIT"
  -sync-mode "$SYNC_MODE"
